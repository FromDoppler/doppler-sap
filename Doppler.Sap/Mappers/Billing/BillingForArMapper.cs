...
namespace Doppler.Sap.Mappers.Billing
{
    public class BillingForArMapper : IBillingMapper
    {
                ...
                foreach (var additionalService in billingRequest.AdditionalServices)
                {
                    switch (additionalService.Type)
                    {
                        ...
                        case AdditionalServiceTypeEnum.PushNotification:
                            {
                                var addOnsItems = GetAddOnItems(billingRequest, currencyCode, additionalService);
                                foreach (SapDocumentLineModel addOnsItem in addOnsItems)
                                {
                                    sapSaleOrder.DocumentLines.Add(addOnsItem);
                                }
        ...
        private List<SapDocumentLineModel> GetAddOnItems(BillingRequest billingRequest, string currencyCode, AdditionalServiceModel additionalService)
        {
            var documentLines = new List<SapDocumentLineModel>();
            var additionalServiceItemCode = string.Empty;
            ...
            var addOnType = additionalService.Type == AdditionalServiceTypeEnum.OnSite ? "OnSite" : "Notificaciones Push";

            var freeText = new
            {
                Amount = $"{currencyCode} {(additionalService.PlanFee > 0 ? additionalService.PlanFee : additionalService.Charge).ToString(CultureInfo.CurrentCulture)} + IMP",
                Periodicity = billingRequest.Periodicity != null ? $"{addOnType} Plan {(Dictionary.PeriodicityDictionary.TryGetValue(billingRequest.Periodicity, out var outPeriodicity) ? outPeriodicity : string.Empty)}" : null,
                Discount = additionalService.Discount > 0 ? $"Descuento {additionalService.Discount}%" : null,
                Payment = billingRequest.Periodicity != null ? $"Abono {billingRequest.PeriodMonth:00} {billingRequest.PeriodYear}" : string.Empty
            };
            ...
            {
                additionalServiceItem.FreeText = $"Doppler - Diferencia por cambio de plan de {addOnType} - {currencyCode} {additionalService.Charge.ToString(CultureInfo.CurrentCulture)}";
            }
            ...
            if (additionalService.ExtraQty > 0)
            {
                ...
                var extraConversationsItem = new SapDocumentLineModel
                {
                    ...
                    FreeText = billingRequest.PlanType == 0 && !string.IsNullOrEmpty(additionalService.UserEmail) ?
                                $"Doppler - {additionalService.ExtraQty}" :
                                $"Doppler - {addOnType} excedentes - {additionalService.ExtraQty}",
                    CostingCode = _costingCode1,
                    CostingCode2 = _costingCode2,
                    CostingCode3 = _costingCode3,
                    CostingCode4 = _costingCode4
